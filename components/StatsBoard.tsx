  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Current Run Statistics</h3>
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-indigo-50 p-4 rounded-lg">
            <p className="text-xs text-indigo-500 font-semibold uppercase">Comparisons</p>
            <p className="text-2xl font-bold text-indigo-700">{currentStats.comparisons}</p>
          </div>
          <div className="bg-red-50 p-4 rounded-lg">
            <p className="text-xs text-red-500 font-semibold uppercase">Swaps</p>
            <p className="text-2xl font-bold text-red-700">{currentStats.swaps}</p>
          </div>
          <div className="bg-green-50 p-4 rounded-lg">
            <p className="text-xs text-green-500 font-semibold uppercase">Est. Time</p>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 min-h-[300px]">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Benchmark Comparison (ms)</h3>
        {benchmarkResults.length === 0 ? (
          <div className="h-full flex items-center justify-center text-gray-400">
            Run different algorithms to compare
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={benchmarkResults} layout="vertical" margin={{ left: 20 }}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="timeMs" fill="#6366f1" name="Time (ms)" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
