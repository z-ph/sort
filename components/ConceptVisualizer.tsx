...
const ConceptVisualizer: React.FC<Props> = ({ 
      ...
      return (
          <div className="flex flex-col items-center w-full gap-8 p-6">
              <div className="space-y-2 w-full">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">当前区间 [{start} - {end}]</p>
              ...
              {isMergeSort && (
                  <div className="space-y-3 w-full pt-4 border-t border-slate-100 dark:border-slate-800">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">辅助数组 (Buffer)</p>
                        <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800"></div>
                      </div>
                      <div className="flex gap-1.5 justify-center min-h-[50px] flex-wrap">
                          {mergeBuffer.length === 0 ? (
                              <div className="text-[10px] text-slate-400 italic">等待数据...</div>
                          ) : (
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative z-20 overflow-visible">
        <div className="bg-slate-50 dark:bg-slate-800/50 px-5 py-3 border-b border-slate-200 dark:border-slate-800 font-black text-slate-700 dark:text-slate-200 text-xs flex items-center justify-between uppercase tracking-wider rounded-t-2xl overflow-visible">
            <div className="flex items-center gap-2">
                <ScanEye size={16} className="text-indigo-500" />
                <span>透视: {algorithm}</span>
            </div>
            <div className="flex items-center gap-1.5 overflow-visible">
                 <button 
                    ...
                    data-tooltip="单步执行"
                    className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:text-indigo-600 transition-all disabled:opacity-30"
                 >
                    <StepForward size={16} />
                 </button>
                 {!isPlaying ? (
                    <button 
                        ...
                        disabled={isFinished}
                        data-tooltip="自动播放"
                        className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-emerald-600 transition-all disabled:opacity-30"
                    >
                    ...
                    <button 
                        onClick={onPause} 
                        data-tooltip="暂停"
                        className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-rose-600 transition-all"
                    >
                 ...
                 <button 
                    onClick={onReset} 
                    data-tooltip="重置"
                    className="p-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:text-rose-600 transition-all"
                 >
                    <RotateCcw size={16} />
                 </button>
                 ...
                 <button 
                    ...
                    data-tooltip={isTracking ? "停用跟随" : "启用跟随"}
                    className={`p-2 rounded-xl transition-all ${isTracking ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'}`}
                 >
                    <Eye size={16} />
                 </button>
            </div>
        </div>
        <div className="min-h-[240px] flex items-center justify-center rounded-b-2xl overflow-auto">
